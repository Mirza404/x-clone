...
| -------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Backend        | Express 4, Mongoose 8, MongoDB, `ts-node`/`nodemon`                          | Single `app.listen(3001)` in `backend/src/index.ts`; app assembled in `backend/src/app.ts`. Routes call controllers, controllers call Mongoose models.    |
...
| Config         | `NEXT_PUBLIC_SERVER_URL` (frontend to backend), `FRONTEND_URL` (CORS origin) | Token minted by `frontend/src/app/api/auth/backend-token/route.ts`.                                                                                       |
...
- Real-time delivery over WebSocket (sender to recipient) without polling.
...
### Non-Goals (v1, noted as future work)
...
| -------------------------- | --------------------------------------- | ------------------------------------ |
| Rooms / per-user targeting | Built-in (`socket.join`, `io.to(room)`) | Hand-rolled map of userId to sockets |
...
- `{ conversation: 1, createdAt: -1 }` is the pagination workhorse.
...
`presence.ts` keeps `Map<userId, Set<socketId>>`. On connect add; on `disconnect` remove; when a user's set transitions between empty and non-empty, emit `presence` to that user's followers/conversation partners (v1: emit to active conversation partners only, keep it cheap). In-memory only; resets on restart, acceptable for v1.
...
| `frontend/src/app/utils/messageApi.ts`                 | REST calls (conversations list, history pagination, get-or-create), mirroring `fetchInfo.ts`                                      |
...
- Sticky sessions at the load balancer (or rely on the Redis adapter + polling-to-websocket upgrade).
