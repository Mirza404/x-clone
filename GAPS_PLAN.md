...
> Renumbering note: these were labelled `A0.1`-`A0.4` before IDEAS.md was restructured. They are now `D1.1`-`D1.4`. Gap 1 is D1.1, Gap 2 is D1.2, Gap 3 is D1.3, Gap 4 is D1.4.
...
## Status note (verified 2026-07-27): Gaps 1 and 2 are built
...
| "Backend Dockerfile: single stage, root"      | **False.** Multi-stage `deps` to `build` to `runner`, `USER node`, `CMD ["node","dist/index.js"]` |
...
1. **Add a build to the backend.**
   ...
   - Add scripts to `backend/package.json`:
     ...
     - set `"start"` to `"node dist/index.js"` (prod). Keep `"dev": "nodemon"` (ts-node) for local.
   ...
   - Env loading caveat: `db/connection.ts` resolves `../../backend/.env` from `__dirname`. Under `dist/` the relative depth changes, so verify the `.env` path resolves in the container (or rely on Docker `env_file`/env vars and drop the file lookup in prod). Flag to test.
...
3. **Env at build vs run.** The Dockerfile already passes a placeholder `MONGODB_URL` at build. `NEXT_PUBLIC_*` vars are inlined at **build** time, so confirm which public vars the runtime needs baked in (e.g. `NEXT_PUBLIC_SERVER_URL`, `NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME`) and pass them as build args, matching the CI `build` job's env list. Server-only vars stay runtime.

4. **`.dockerignore`:** already ignores `.next`, which is good (it forces a clean build). Add `coverage`, `.git`.
...
- Missing `public/` copy means 404s on static assets. Verify the dir.
- A `NEXT_PUBLIC_*` var not present at build time gives a wrong/blank value at runtime. Keep the build-arg list in sync with CI.
...
2. `test:ci` already runs jest, so coverage rides along. Add `--coverage` explicitly if not auto.
